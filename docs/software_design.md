...
W-correlation 修复后仍**不如不加**（baseline 13.5dB vs best 13.4dB）。根本原因：硬截断框架下额外过滤只会减少信号。

详见 [实验日志](experiment_log.md#2026-05-08-w-correlation-修复与实验)。

**决策**：W-correlation 不应默认启用。探索 Wiener 软加权替代硬截断。

#### Wiener 软加权

用连续权重替代 0/1 截断。Wiener 增益：

```
w_i = max(0, 1 - σ_noise² / σ_i²)
```

实测 Wiener nf=0.1 SNR 43.9dB vs 能量截断 16.0dB（+27.9dB）。详见 [实验日志](experiment_log.md#2026-05-08-wiener-软加权)。
...
**方向**：Wiener 软加权（连续权重替代 0/1 决策）已在实验中验证优于硬截断。
