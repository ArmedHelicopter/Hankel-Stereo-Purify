...
### 5.3 W-correlation、W 权重与分组

- **W 权重**：在 SSA/MSSA 文献中，对轨迹矩阵行（或等价地，对重构分量）定义加权内积时，权重由窗长 \(L\)、序列长度与对角平均的重叠度共同决定，使不同索引的分量在统计上可比；**W-correlation** 即在该加权度量下分量之间的相关性。
- **分组**：`strategies/grouping.py` 等路径依据 W-correlation 将奇异分量聚类后再对角重构，减轻错误分离带来的伪影；阈值与分组策略由配置与数据决定。
