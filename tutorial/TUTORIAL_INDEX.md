...
| 复述 OLA 与短时平稳假设 | 跟读 `list_frame_starts`、`SoundfileOlaEngine` 中 OLA 主循环（`AudioPurifier` 组合引擎） | 指认 `pcm_producer` 背压、memmap、`Queue` | 05 |
| 区分 `ConfigurationError` / `AudioIOError` / `ProcessingError`；并读出包装前的**异常类型** | 从 `cli.py` 追到 `process_file`；`ProcessingError.origin_exception_type`（`exception_fully_qualified_name`） | 指认 `validate_io_paths`；`test_purifier_producer_errors` / `test_exceptions` | 06 |
...
**运行代码前**：在项目根目录设置 `PYTHONPATH=src`，依赖与命令见仓库根目录主说明。
...
| 系统库 | `soundfile` 依赖本机已安装的 **libsndfile**（各发行版包名不同；排障见仓库根目录主说明） |
...
- **实现**：在 IDE 中从 `_make_denoise_frame_fn` 跳到 `make_svd_step`，用注释标出固定秩分支与能量分支的分叉条件。
