...
**为什么 memmap 累加器？**  
当样本数 × 每样本字节超过 `--max-memory-mb` 预算，累加缓冲落到临时文件映射，换 **I/O 带宽** 换 **RAM**——见 [`_allocate_ola_accumulators`](../src/facade/purifier.py)。
...
| 整文件 OLA 主循环、memmap 累加器 | `_allocate_ola_accumulators`、处理循环 | [`src/facade/purifier.py`](../src/facade/purifier.py) |
...
- **晦涩**：`purifier.py` 单文件较长，初学者易迷失在线程与 OLA 之间。  
- **建议**：先画「主线程：OLA 循环；子线程：读块」的时序图，再对照 `_run_processing_soundfile`。
