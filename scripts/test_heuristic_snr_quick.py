"""Quick SNR test for heuristic weighting."""
...
def test_snr():
    """Test SNR for different weighting strategies."""
    ...
    # Test strategies
    print("\n策略对比:")
    print(f"{'策略':<25} {'SNR (dB)':<15} {'保留组件':<15}")
    print("-" * 55)
    ...
    print(f"{'Energy (0.9)':<25} {snr_energy:<15.2f} {k_energy:<15}")
    ...
    print(f"{'Wiener (0.1)':<25} {snr_wiener:<15.2f} {np.sum(wiener_weights > 0.1):<15}")
    ...
    print(f"{'Heuristic':<25} {snr_heuristic:<15.2f} {np.sum(heuristic_weights > 0.1):<15}")
    ...
    for sfm_low, sfm_high in [(0.1, 0.5), (0.2, 0.6), (0.3, 0.7)]:
        ...
        print(f"  SFM阈值({sfm_low}-{sfm_high}): SNR={snr_tuned:.2f}dB, 保留组件={np.sum(weights_tuned > 0.1)}")


if __name__ == "__main__":
    print("测试启发式多特征加权策略SNR")
    print("=" * 50)
